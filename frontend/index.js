...
function moduleProject1() {
  ...
  // 👉 TASK 2 - Build a "Quote of the Day" widget b
  //  ✨ add your code here
  ...
  authorDate.textContent = `${author} in ${date || "an uknown date"}`;
  ...
  const randomAdverb1 = verbs[Math.floor(Math.random() * nouns.length)];
  const randomAdverb2 = adverbs[Math.floor(Math.random() * adverbs.length)];
  const randomVerb1 = verbs[Math.floor(Math.random() * verbs.length)];
  const randomVerb2 = verbs[Math.floor(Math.random() * verbs.length)];

const mumboJumbo = `We need to ${randomVerb1 } our ${randomNouns1} ${randomAdverb1} in order to ${randomVerb1} our ${randomNouns1} ${randomAdverb1}.`
